...
`git clone https://github.com/ByteLegendQuest/java-common-classes-hierarchies`

A project directory named `java-common-classes-hierarchies` will be created, which can be moved wherever you like.

Open `IDEA`, `File` - `Open`, select and open the `pom.xml` in `java-common-classes-hierarchies` directory which is created just now.
